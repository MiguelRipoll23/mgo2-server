...
# deployment.env instead, which compose feeds to every container as environment
# variables; the environment overrides appsettings.json. The secret is written
# once, the address is detected again on every run so an update follows a
# machine that changed networks, and ADVERTISED_ADDRESS skips the detection.
#
