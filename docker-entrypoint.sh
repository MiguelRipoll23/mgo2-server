...
su -s /bin/sh postgres -c "pg_ctl -D '$PGDATA' -l '$PG_LOG' start"

# Wait for PostgreSQL to be ready
echo "[entrypoint] Waiting for PostgreSQL..."
until su -s /bin/sh postgres -c "pg_isready -q"; do
  sleep 1
done

# Create database if it doesn't exist
su -s /bin/sh postgres -c "psql -tc \"SELECT 1 FROM pg_database WHERE datname='mgo2'\" | grep -q 1 || createdb mgo2"
