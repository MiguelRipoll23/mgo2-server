...
| `0xD32E08` / `0xD32E3C` / `0xD32E70` / `0xD330C4` | the **wait-state API**, fully characterised 2026-08-03 (batch 5b, `0x2007` pass). `0xD32E08` = `SetWaitState(session, slot, state)`: a single guarded `stw` to `session+360+4*slot` (slot ≤ 116, state ≤ 2) — **NOT a notify**; nothing is signalled, nothing queues. `0xD32E3C` get-state (default 4), `0xD32E70` set-result → `session+828+4*slot`, `0xD330C4` get-result (default −24). Per-connection resets `0xD32E9C` partition the 117 slots: **GATE 10..12, ACCOUNT 13..20, GAME 21..116**. Generic waiter `0xD35834` (requires state 2, clears, returns −1), wrapped by per-slot thunks in the `0xD360xx` bank |
| `0xD34FF0` | the **receive pump** — `select` at `0xD34DF0`, recv at `0xD34D1C`; hands the SAME session pointer to all three dispatchers (`0xD351D4` GATE, `0xD351E4` ACCOUNT, `0xD351F4` GAME). The session singleton is `*(*(*(r2−31404)−32768))`, **75,608 bytes** (memset in init `0xD355B4`); its first three 68-byte records are the GATE/ACCOUNT/GAME connections (fd at +0), wait arrays at +360/+828, three 1152-byte receive buffers via `*(+71944)` |
| `0xD37B00` / `0xD36900` | **dead command builders, 2026-08-03**: `0x3040` (activate character by slot; `0xD37B6C` is the `li r4` inside it) and `0x2006` (empty payload, reply `0x2007`). Both zero callers image-wide, control-validated in their own OPD banks — neither is sendable by the disc build. See `PACKETS_NOT_OBSERVED.md` and the four `.ksy` files |
