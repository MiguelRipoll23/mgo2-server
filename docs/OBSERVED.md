...
**2026-09-18 — the presence table is ported, and with it the three readers that had no answer to
give.** `character_presence` (migration `CharacterPresence`, generated by the tool like every
other) records which lobby each character is in, one row per character so a lobby change is one
upsert, and the row is claimed by the destination and released **only by the lobby that still owns
it** — a hop is two processes racing, and a late disconnect from the origin would otherwise erase
the arrival and make the player flicker out of every friend list. A process clears its own lobby's
rows at startup, which is exact rather than heuristic because nobody is connected to a process that
has just started; a 30-second beat keeps its live rows fresh and a 120-second sweep clears the rows
of a process that died and never came back. This is the reference's design unchanged — its
`V72`/`PresenceService` — with this server's naming and hooks.

**Two of the three readers were also wrong about the record, not just about the value.** The friend
roster wrote 43-byte entries: a per-row state byte, the target, the name, an online flag, three
bytes of padding, and the lobby id and name at the end. The record is **59 bytes and has no state
byte** — the state is the transaction, keyed by the `{u8 state}` the client asked with — and its
tail is the same five-field location block the player search and the clan roster carry. The search
record was 59 bytes by accident: it wrote an online flag borrowed from the roster's vocabulary,
which put every field boundary one byte off, and the trailing label byte fell off the end. **A
record of the right length is not a record of the right layout**, which is why the roster survived
long enough to be seen on screen at all. Both now write the block through one writer, and the
...
offline friend keeps their row and the client draws its `"----"` placeholder in the lobby column.
`0x4602` does no compaction at all. Nothing is gated on the block, which is what lets a search
result for a player who is not connected stay honest and still be a result.
