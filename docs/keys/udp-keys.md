...
description lives in `../udp-p2p-protocol.md`; this file is the quick reference.
...
> (see "Wire prefix" below and `../udp-p2p-protocol.md` §3/§5).
...
|---|---|---|---|---|
| `0x00` | u16 | **scrambled header** — unscrambles to the per-frame counter (`LE` u16); seeds the XOR chain | LE | see `../udp-p2p-protocol.md` §5.1 |
...
reproduces the exact wire bytes. See `../udp-p2p-protocol.md` §5 for the annotated version.
...
- **The tail is a self-verifying MD5 digest, validated on every datagram** (decoder
  ...
  applies it after the chain and before the scramble (§5.3 of `../udp-p2p-protocol.md`).
