...
/// <summary>
/// Guards the pairing listing the event testing tools read. A pairing is only
/// announced to its teams once a room has been leased for it, so this is the
/// only place a pair that has matched — or a pair still waiting for a host —
/// can be seen at all, and a query that does not compile leaves the page
/// reporting no pairings rather than the ones that exist.
/// </summary>
[Trait("Category", "Shared")]
public sealed class EventMatchListingTests
{
    [Fact]
    public async Task The_listing_compiles_against_the_provider()
    {
        var service = new EventMatchListingService(new UnreachableDbContextFactory());
    ...
    [Fact]
    public async Task A_lobby_with_no_identifier_lists_no_pairings_without_touching_the_database()
    {
        ...
        var service = new EventMatchListingService(factory);

        var matches = await service.ListAsync(0, CancellationToken.None);
